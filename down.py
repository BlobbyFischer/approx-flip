...
if FOLDER_NAME is None:
    print("Usage: python alldown.py <folder> <order> <timeout> (<processors>)")
    sys.exit(1)

FLIP_TIMEOUT = int(sys.argv[3]) if len(sys.argv) > 3 else None

if FLIP_TIMEOUT is None:
    print("Usage: python alldown.py <folder> <order> <timeout> (<processors>)")
    sys.exit(1)

ORDER = int(sys.argv[2]) if len(sys.argv) > 2 else None
...
TOTAL_NO_REDUCTION_FLIPS_THRESHOLD = 100 # Total ./flip calls (across all threads) that yield no reduction before moving to next size.
TOTAL_REDUCTIONS_FOUND_THRESHOLD = 20 # Number of *distinct rank reductions* found before moving to the new target rank.

def find_lowest_rank_dir():
    base_path = Path("solutions") / f"{FOLDER_NAME}"
    if not base_path.exists():
        return ""
    lowest_rank = float("inf")
    lowest_path = ""
    for path in sorted(base_path.glob(f"*")):
        if path.is_dir():
            try:
                rank = int(path.name)
                if rank < lowest_rank:
                    lowest_rank = rank
                    lowest_path = str(path)
            except ValueError:
                continue
    return lowest_path
...
try:
    current_rank = [int(current_dir[0].name)]
except:
    print("Couldn't get rank from folder name")
    sys.exit(1)
...
def flip_worker(thread_id, home_dir):
    while True:
        #choose a file
        with lock[0]:
            input_file = get_random_file_from_dir(base_dir / f"{current_rank[0]}")
            #do flip
        proc = subprocess.Popen([f"./flip{ORDER+1}", input_file, str(1000000), "0", "1"], stdout=subprocess.PIPE, stderr=subprocess.PIPE, text=True)
        ...
        with lock[0]:
            if new_rank < current_rank[0]:
                ...
                if reductions[0] >= TOTAL_REDUCTIONS_FOUND_THRESHOLD:
                    current_rank[0] -= 1
                    current_dir[0] = Path("solutions") / f"{FOLDER_NAME}" / f"{current_rank[0]}"
                    reductions[0] = 0
                    failed_attempts[0] = 0
                    print(f"--Trying to reduce from rank {current_rank[0]}--")
            #else stop all workers if too many things
            else:
                shutil.move(new_file_path, current_dir[0] / new_file_path.name)
                failed_attempts[0] += 1
